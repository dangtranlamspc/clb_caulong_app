...
function AdminAddTournamentRegistrationModal({
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "guest" && !guestEmail.trim()) {
      toast.error("Vui lòng nhập email để gửi QR thanh toán cho khách");
      return;
    }
          ...
          <FadeSwitch transitionKey={mode}>
            {mode === "member" ? (
              ...
              <div className="space-y-3">
                ...
                <Field label="Email (để gửi QR thanh toán)" required>
                  <input
                    type="email"
                    className="input-field transition-all duration-200 focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400"
                    value={guestEmail}
                    ...
                    {r.payment_status !== "confirmed" && r.payment_method ? (
                      <div className="flex items-center gap-2 flex-1">
                        <button
                          onClick={() => handleConfirmPayment(r.id)}
                          disabled={payingAction?.id === r.id}
                          className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg bg-green-600 text-white text-sm font-semibold hover:bg-green-700 disabled:opacity-60 transition-colors"
                        >
                          {payingAction && payingAction.id === r.id && payingAction.type === "confirm" ? (
                            <Loader2 className="w-4 h-4 animate-spin" />
                          ) : (
                        ...
                        <button
                          onClick={() => handleRejectPayment(r.id)}
                          disabled={payingAction?.id === r.id}
                          className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg bg-red-600 text-white text-sm font-semibold hover:bg-red-700 disabled:opacity-60 transition-colors"
                        >
                          {payingAction && payingAction.id === r.id && payingAction.type === "reject" ? (
                            <Loader2 className="w-4 h-4 animate-spin" />
                          ) : (
