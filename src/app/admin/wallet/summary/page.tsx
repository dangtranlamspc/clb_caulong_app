...
function MemberMobileCard({ m, active, onSelect, delay }: { m: any; active: boolean; onSelect: () => void; delay: number }) {
    return (
        <button
            onClick={onSelect}
        ...
        >
            ...
            <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                    ...
                    <span className={`text-sm font-bold whitespace-nowrap flex-shrink-0 ${m.balance < 0 ? 'text-red-500' : 'text-gray-900'}`}>
                        {fmt(m.balance)}
                    </span>
                </div>
                ...
                <p className="text-[11px] text-gray-400 mt-1.5">Buổi gần nhất: {relativeDay(m.last_session_at)}</p>
            </div>
        </button>
    );
}
                        ...
                        >
                            ...
                            <div className="sm:hidden flex flex-col gap-2.5 px-3 py-3">
                                {loading ? (
                                    ...
                                    <div className="px-4 py-12 text-center text-gray-400 text-sm">Không tìm thấy thành viên</div>
                                ) : members.map((m, i) => (
                                    <MemberMobileCard
                                        key={m.id}
                                        m={m}
                                        ...
                                        onSelect={() => setSelectedMember(selectedMember?.id === m.id ? null : m)}
                                    />
                                ))}
                            </div>
                        </div>
