...
import {
    ...
    CheckCircle2, Hourglass, AlertCircle, Copy,
    Loader2, XCircle, X as XIcon,
    UserPlus, Wallet,
    Download,
} from 'lucide-react';
...
function getPaymentMethodBadge(reg: any) {
    ...
    if (m === 'wallet' || m === 'wallet_grouped') {
        return { label: 'Ví BNB', icon: <Wallet className="w-2.5 h-2.5" />, cls: 'bg-blue-100 text-blue-700' };
    }
    if (m === 'grouped_with_host') {
        return { label: 'Chuyển khoản', icon: <span>🏦</span>, cls: 'bg-sky-100 text-sky-700' };
    }
    if (m === 'cash') {
        return { label: 'Tiền mặt', icon: <span>💵</span>, cls: 'bg-emerald-100 text-emerald-700' };
    }
...
function WalletGuestConfirmModal({
    ...
    const hoursLeft = Math.max(0, Math.floor((deadlineDate.getTime() - Date.now()) / 3600000));
        ...
        >
            ...
            >
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                    <div>
                        <p className="text-sm font-bold text-gray-900">Xác nhận thanh toán khách đi cùng</p>
                ...
                <div className="px-5 py-4 space-y-4">
                    ...
                    <div className="bg-blue-50 rounded-xl px-4 py-3 flex items-start gap-3">
                        ...
                        <div>
                            <p className="text-sm font-semibold text-blue-900">Phần của bạn đã được trừ ví</p>
                            <p className="text-xs text-blue-600 mt-0.5">
                                Khách đi cùng ({guestNames}) cần thanh toán <strong>{fmt(guestTotal)}</strong>. Bạn muốn gộp chung vào ví không?
                            </p>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-gray-900">Trừ thêm vào ví của tôi</p>
                            <p className="text-lg font-black text-blue-600">{fmt(guestTotal)}</p>
                            <p className="text-xs text-gray-400 mt-0.5">
                                Gộp tiền cho {guestNames} vào ví BNB của bạn
                            </p>
                        </div>
                        {submitting && <Loader2 className="w-4 h-4 animate-spin text-blue-600 flex-shrink-0" />}
                    </button>
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-gray-900">Khách tự thanh toán tiền mặt</p>
                            <p className="text-xs text-gray-400 mt-0.5">
                    ...
                    <p className="text-xs text-gray-400 text-center">
                        Nếu không xác nhận trước {format(deadlineDate, 'HH:mm dd/MM', { locale: vi })}, hệ thống sẽ tự động trừ ví gộp chung.
                    </p>
                </div>
            </div>
        </div>,
...
export default function SessionDetailPage() {
    ...
    const handleCancel = async () => {
        ...
        try {
            ...
            setTimeout(() => { fetchSession(); fetchRegistrations(); setCancelPhase('idle'); }, 700);
        } catch (err: any) {
            setCancelPhase('idle');
    ...
    const handleAddGuest = async () => {
        if (!myReg?.id || !guestForm.full_name.trim()) { toast.error('Vui lòng nhập họ tên khách'); return; }
        setAddingGuest(true);
        try {
    ...
    if (!session) return <div className="text-center py-20 text-gray-400">Không tìm thấy buổi đánh</div>;
    ...
    const myGuests = registrations.filter((r: any) => r.host_registration_id === myReg?.id);
    const guestTotal = myGuests.reduce((sum: number, g: any) => sum + (g.amount_override ?? 0), 0);
    ...
    const confirmedParticipants = registrations.filter((r: any) => r.participation_status === 'confirmed');
    const allParticipantsPaid = confirmedParticipants.length > 0
        && confirmedParticipants.every((r: any) => r.payment_status === 'confirmed');
    const isAwaitingAdminFinish = session.status === 'waiting_payment' && allParticipantsPaid;
        ...
        <>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                    <button
                        onClick={() => { sessionStorage.setItem('activity:return-tab', 'sessions'); router.push('/activity'); }}
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm">
                    <div className="flex items-start justify-between gap-2 mb-4">
                        ...
                        <span className={`text-xs px-2.5 py-1 rounded-full font-medium flex-shrink-0 ${effectiveStatus === 'open' ? 'bg-emerald-50 text-emerald-700' :
                            effectiveStatus === 'full' ? 'bg-amber-50 text-amber-700' :
                                isAwaitingAdminFinish ? 'bg-blue-50 text-blue-700' :
                    ...
                    {session.description && (
                        <p className="mt-4 pt-4 border-t border-gray-50 text-sm text-gray-500 leading-relaxed">{session.description}</p>
                    )}
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm">
                    ...
                    {loadingRegs ? (
                        ...
                        const guestsOf = (hostId: string) => registrations.filter((m: any) => m.host_registration_id === hostId);
                        const renderPerson = (m: any, opts?: { nested?: boolean }) => {
                            ...
                            return (
                                <li key={m.id} className={`flex items-center gap-3 ${opts?.nested ? 'py-2 pl-6' : 'py-2.5'}`} style={{ animation: 'fadeSlideUp .3s ease both' }}>
                                    ...
                                    {u?.avatar_url ? (
                                        ...
                                        <div className={`rounded-full flex items-center justify-center flex-shrink-0 font-semibold ${opts?.nested ? 'w-7 h-7 text-[10px] bg-purple-100 text-purple-700' : 'w-9 h-9 text-xs bg-blue-100 text-blue-700'}`}>
                                            {initials}
                                        </div>
                                    )}
                                    <div className="flex-1 min-w-0">
                                        <p className={`font-medium text-gray-900 truncate ${opts?.nested ? 'text-xs' : 'text-sm'}`}>
                                            {fullName}{m.is_guest && <span className="text-xs text-gray-400 ml-1">(khách)</span>}
                                        </p>
                            ...
                            <ul className="divide-y divide-gray-50">
                                {roots.map((root: any) => {
                                    ...
                                    return (
                                        <div key={root.id}>
                                            {renderPerson(root)}
                                            {guests.length > 0 && <ul>{guests.map((g: any) => renderPerson(g, { nested: true }))}</ul>}
                                        </div>
                                    );
                                })}
                            </ul>
                ...
                {(session.status === 'waiting_payment' || session.status === 'completed') &&
                    (session.court_fee > 0 || session.shuttle_count > 0 || totalOtherFeeFromRegs > 0) && (
                        <div className="bg-white rounded-2xl p-5 shadow-sm space-y-3" style={{ animation: 'fadeSlideUp .3s ease both' }}>
                            <h3 className="font-bold text-gray-900 text-sm flex items-center gap-2">🧾 Chi phí buổi đánh</h3>
                            <div className="space-y-1.5 text-sm">
                                {session.shuttle_count > 0 && (
                                    <div className="flex justify-between text-gray-600">
                                        <span>🏸 Cầu {session.shuttle_count} × {fmt(session.shuttle_price ?? 0)}</span>
                                        <span className="font-medium">{fmt((session.shuttle_count ?? 0) * (session.shuttle_price ?? 0))}</span>
                                    </div>
                                )}
                                {session.court_fee > 0 && (
                                    <div className="flex justify-between text-gray-600">
                                ...
                                <div className="flex justify-between font-bold border-t border-gray-100 pt-2 text-gray-900">
                                    ...
                                    <span>{fmt((session.court_fee ?? 0) + (session.shuttle_count ?? 0) * (session.shuttle_price ?? 0) + totalOtherFeeFromRegs)}</span>
                                </div>
                            </div>
                                                    ...
                                                    <div className="flex justify-between items-center">
                                                        <span className={`text-sm font-medium ${isMe ? 'text-blue-700' : 'text-gray-700'}`}>
                                                            ...
                                                            {(() => {
                                                                ...
                                                                return (
                                                                    <span className={`ml-1.5 inline-flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded-full font-semibold ${badge.cls}`}>
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            {r.payment_status === 'confirmed' ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> : <Hourglass className="w-3.5 h-3.5 text-amber-400" />}
                ...
                {myReg && myReg.participation_status === 'awaiting_checkin' && (
                    <div className="bg-white rounded-2xl p-5 shadow-sm flex items-center gap-3" style={{ animation: 'fadeSlideUp .35s ease both' }}>
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">Admin sẽ điểm danh khi buổi bắt đầu. Sau khi buổi kết thúc, số tiền cần thanh toán sẽ hiện ở đây.</p>
                        </div>
                    </div>
                )}
                ...
                {myReg && myReg.participation_status === 'confirmed' && myReg.payment_status === 'pending' && !myReg.amount_override && (
                    <div className="bg-white rounded-2xl p-5 shadow-sm flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center flex-shrink-0"><Hourglass className="w-5 h-5 text-blue-500" /></div>
                        <div>
                            <p className="text-sm font-semibold text-gray-900">Đã điểm danh có mặt</p>
                            <p className="text-xs text-gray-500 mt-0.5">Số tiền cần thanh toán sẽ được admin thông báo sau khi buổi kết thúc.</p>
                        </div>
                    </div>
                )}
                ...
                {myReg && hasPendingGuestWallet && myReg.payment_status === 'confirmed' && (
                    <button
                        onClick={() => {
                            const pendingGuest = myGuests.find((g: any) => g.payment_method === 'wallet_pending_confirm');
                            if (!pendingGuest) return;
                            setGuestConfirmPayload({
                                hostRegistrationId: myReg.id,
                                guestNames: myGuests.filter((g: any) => g.payment_method === 'wallet_pending_confirm').map((g: any) => g.guest_full_name).join(', '),
                                guestTotal: myGuests.filter((g: any) => g.payment_method === 'wallet_pending_confirm').reduce((s: number, g: any) => s + (g.amount_override ?? 0), 0),
                                deadline: pendingGuest.wallet_confirm_deadline ?? new Date(Date.now() + 86400000).toISOString(),
                            });
                        }}
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-amber-800">Cần xác nhận thanh toán cho khách đi cùng</p>
                            <p className="text-xs text-amber-600 mt-0.5">Nhấn để chọn gộp ví hoặc để khách tự nộp tiền mặt →</p>
                        </div>
                    </button>
                )}
                ...
                {myReg && myReg.payment_status === 'rejected' && (
                    <div className="bg-white rounded-2xl p-5 shadow-sm flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center flex-shrink-0"><AlertCircle className="w-5 h-5 text-red-500" /></div>
                        <div className="flex-1">
                            <p className="text-sm font-semibold text-gray-900">Thanh toán bị từ chối</p>
                            <p className="text-xs text-gray-500 mt-0.5">Vui lòng liên hệ admin để được hỗ trợ thanh toán lại.</p>
                        </div>
                    </div>
                )}
                ...
                {/* Guest modal */}
                {showGuestModal && (
                    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4" onClick={(e) => e.target === e.currentTarget && closeGuestModal()}>
                        <div className="w-full max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-xl">
                            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                                ...
                                <button onClick={closeGuestModal} className="p-1 text-gray-400 hover:text-gray-600"><XIcon className="w-5 h-5" /></button>
                            </div>
                            <div className="p-5 space-y-3">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Họ tên *</label>
                                    <input autoFocus value={guestForm.full_name} onChange={e => setGuestForm(f => ({ ...f, full_name: e.target.value }))} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="Tên khách đi cùng" />
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Giới tính</label>
                                        <select value={guestForm.gender} onChange={e => setGuestForm(f => ({ ...f, gender: e.target.value }))} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-400">
                                    ...
                                    <div>
                                        ...
                                        <select value={guestForm.skill_level} onChange={e => setGuestForm(f => ({ ...f, skill_level: e.target.value }))} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-blue-400">
                                            {SKILL_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                                        </select>
                                    </div>
                                </div>
                                <p className="text-xs text-blue-600 bg-blue-50 rounded-lg px-3 py-2">ⓘ Tiền của khách đi cùng sẽ được gộp vào số tiền bạn cần thanh toán sau khi buổi kết thúc.</p>
                            </div>
                            <div className="flex justify-end gap-3 px-5 py-4 border-t border-gray-100">
                                <button onClick={closeGuestModal} className="flex-1 sm:flex-none py-2.5 px-4 rounded-xl border border-gray-200 text-sm text-gray-500 hover:bg-gray-50">Hủy</button>
                                <button onClick={handleAddGuest} disabled={addingGuest || !guestForm.full_name.trim()} className="flex-1 sm:flex-none py-2.5 px-4 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-1.5">
                                    {addingGuest && <Loader2 className="w-4 h-4 animate-spin" />} Thêm khách
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-2" style={{ animation: 'fadeSlideUp .35s ease both', animationDelay: '80ms' }}>
                    {canRegister && <MorphButton phase={registerPhase} label="🏸 Đăng ký tham gia" onClick={handleRegister} />}
                    ...
                    {myReg && myReg.payment_status === 'pending' && !myReg.amount_override && !myReg.added_by_user_id && (
                        <MorphButton phase={cancelPhase} label="Hủy đăng ký" idleIcon={<XCircle className="w-3.5 h-3.5" />} onClick={handleCancel} idleClassName="bg-white border border-red-200 text-red-500 hover:bg-red-50" successClassName="bg-red-500 text-white" />
                    )}

                    {isFull && !myReg && (
                        <div className="w-full py-4 rounded-2xl bg-gray-100 text-gray-400 font-medium text-center text-sm">Buổi đã đầy chỗ</div>
                    )}
                </div>

                {/* Pay modal */}
                {showPayModal && myReg && typeof document !== 'undefined' && createPortal(
                    <div
                        className="fixed inset-0 flex flex-col justify-end"
                        style={{
                            zIndex: 99999,
                            background: payModalVisible ? 'rgba(0,0,0,0.5)' : 'rgba(0,0,0,0)',
                                    ...
                                    <p className="text-sm font-bold text-gray-900">
                                        {!payType ? 'Chọn hình thức thanh toán' :
                                            payMethod === 'choose' ? 'Chọn phương thức' :
                                                payMethod === 'transfer' ? 'Chuyển khoản' :
                                                    payMethod === 'wallet' ? 'Trừ ví BNB' : 'Tiền mặt'}
                                    </p>
                                    <p className="text-xs text-gray-400 mt-0.5">{session.title}</p>
                                </div>
                                <button onClick={closePayModal} className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center"><XIcon className="w-4 h-4 text-gray-500" /></button>
                            </div>
                            ...
                            <div className="px-5 py-4 space-y-4">
                                {!payType && myGuests.length > 0 && (
                                    <div className="space-y-3" style={{ animation: 'fadeSlideUp .25s ease both' }}>
                                        ...
                                        <button onClick={() => { setPayType('solo'); setPayMethod('choose'); }} className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-200 hover:border-blue-400 hover:bg-blue-50 transition-colors text-left">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-400">Khách đi cùng ({myGuests.map((g: any) => g.guest_full_name).join(', ')}) tự thanh toán riêng</p>
                                            </div>
                                        </button>
                                        <button onClick={() => { setPayType('grouped'); setPayMethod('choose'); }} className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-200 hover:border-purple-400 hover:bg-purple-50 transition-colors text-left">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-400">Bao gồm: {myGuests.map((g: any) => `${g.guest_full_name} (${fmt(g.amount_override ?? 0)})`).join(', ')}</p>
                                            </div>
                                        </button>
                                    </div>
                                )}

                                {!payType && myGuests.length === 0 && (() => { setPayType('solo'); return null; })()}
                                        ...
                                        <div key={payType} style={{ animation: 'fadeSlideUp .25s ease both' }}>
                                            ...
                                            {payMethod === 'choose' && (
                                                <div className="space-y-3 mt-4" key="choose" style={{ animation: 'fadeSlideUp .25s ease both' }}>
                                                    {/* Ví BNB */}
                                                    <button onClick={() => setPayMethod('wallet')} className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-200 hover:border-blue-500 hover:bg-blue-50 transition-colors text-left">
                                                        <div className="w-11 h-11 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                                                            <Wallet className="w-5 h-5 text-blue-600" />
                                                        </div>
                                                        <div>
                                                    ...
                                                    {/* Chuyển khoản */}
                                                    <button onClick={() => setPayMethod('transfer')} className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-200 hover:border-blue-400 hover:bg-blue-50 transition-colors text-left">
                                                        <div className="w-11 h-11 rounded-full bg-blue-100 flex items-center justify-center text-xl">🏦</div>
                                                        <div>
                                                    ...
                                                    {/* Tiền mặt */}
                                                    <button onClick={() => setPayMethod('cash')} className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-200 hover:border-green-400 hover:bg-green-50 transition-colors text-left">
                                                    ...
                                                    {myGuests.length > 0 && (
                                                        <button onClick={() => setPayType(null)} className="w-full py-2 text-xs text-gray-400 hover:text-gray-600">← Quay lại chọn kiểu thanh toán</button>
                                                    )}
                                                </div>
                                            )}
                                                ...
                                                return (
                                                    <div className="space-y-4 mt-4" key="transfer" style={{ animation: 'fadeSlideUp .25s ease both' }}>
                                                        <div className="bg-white border-2 border-gray-100 rounded-2xl p-4 flex flex-col items-center gap-2">
                                                            ...
                                                            <img src={qr} alt="VietQR" className="w-48 h-48 object-contain" onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                                                        </div>
