...
function TopupModal({
  ...
  const handleAmountChange = (raw: string) => {
    ...
    setAmount(num);
    setAmountDisplay(num ? num.toLocaleString("vi-VN") : "");
  };

  const selectQuickAmount = (v: number) => {
    setAmount(v);
    setAmountDisplay(v.toLocaleString("vi-VN"));
  };

  const suggestedRef = `NAPVI ${stripDiacritics(user?.full_name ?? "THANHVIEN")} ${format(new Date(), "ddMMyyyy")}`;
  const qrUrl =
    amount > 0
  ...
  const handleSubmitTransfer = async () => {
    ...
    try {
      await walletApi.requestTopup({
        ...
        payment_reference: suggestedRef,
        payment_proof_url: billUrl ?? undefined,
        note: note || undefined,
      });
